...
export function validateSetup(input) {
  ...
  if (!definition.modelOptional && !clean(input.model, 300)) {
    throw new Error("A conversation model ID is required.");
  }
  ...
  return {
    ...
    model: clean(input.model, 300) || definition.defaultModel,
    baseUrl,
...
export function buildEnvText(config) {
  ...
  if (definition.keyEnv) values[definition.keyEnv] = config.primaryApiKey;
  if (definition.modelEnv) values[definition.modelEnv] = config.model;
  return (
